...
def compute_forward(states, observations, transitions, emissions):
    ...
    for t in inclusive_range(2, big_t):
        for s in inclusive_range(1, big_n):
            forward[s, t] = sum(forward[s_prime, t - 1] * transitions[s_prime, s] * emissions[s, observations[t]]
                                for s_prime in inclusive_range(1, big_n))

    forward[f, big_t] = sum(forward[s, big_t] * transitions[s, f]
                            for s in inclusive_range(1, big_n))

    return forward[f, big_t]

    # for r in inclusive_range(forward[s,t]):
    ##    forward[r,t-1] = transitions(s_mark,s) * emissions(observations[t])
...
def compute_viterbi(states, observations, transitions, emissions):
    ...
    for t in inclusive_range(2, big_t):
        for s in inclusive_range(1, big_n):
            viterbi[s, t] = max(viterbi[s_prime, t - 1] * transitions[s_prime, s] * emissions[s, observations[t]]
                                for s_prime in inclusive_range(1, big_n))
            backpointers[s, t] = argmax(viterbi[s_prime, t - 1] * transitions[s_prime, s]
                                        for s_prime in inclusive_range(1, big_n))

        viterbi[f, big_t] = max(viterbi[s, big_t] * transitions[s, f]
                            for s in inclusive_range(1, big_n))
        backpointers[f, big_t] = argmax(viterbi[s, big_t] * transitions[s, f]
                                    for s in inclusive_range(1, big_n))
